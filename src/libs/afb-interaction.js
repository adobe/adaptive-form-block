...
/**
 *
 * @param {{ querySelector: (arg0: string) => any; }} element
 * @param {any} errorMessage
 * @param {{ errorMessage: any; }} state
 */
export const updateErrorMessage = (element, errorMessage, state) => {
  if (element) {
    const errorDiv = getErrorWidget(element);
...
export const subscribe = (model, element, additionalHandlers) => {
  const handlers = { ...defaultHandlers(), ...additionalHandlers };
