...
export default class ExcelToJsonFormula {
  numberRegEx = /\d+/;
  ...
  visit(n, v) {
    const visitFunctions = {
      ...
      OrExpression: (node) => {
        ...
        if (isFalse(matched)) matched = this.visit(node.children[1]); // eslint-disable-line no-undef
        return matched;
      },
      ...
      AndExpression: (node) => {
        ...
        if (isFalse(first) === true) return first; // eslint-disable-line no-undef
